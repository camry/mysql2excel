...
func (td *TableDiff) Run() error {
    ...
    for _, table := range targetTableList {
        ...
        if !lo.HasKey(td.sourceTableMap, table.TableName) {
            deleteTableList = append(deleteTableList, table)
        }
    }

    // DEL
    deleteTableChunkList := lo.Chunk(deleteTableList, 10)
    for _, tableChunk := range deleteTableChunkList {
        ...
        for _, table := range tableChunk {
            ...
            if err != nil {
                glog.Fatal(gerror.Wrapf(err, "dumpCmd TableName %s Count Failed", table.TableName))
            }
            if count > 0 {
            ...
            go td.delTable(bar, wg, errChan, table, limit, page)
        }
        progress.Wait()
        close(errChan)
...
func (td *TableDiff) delTable(bar *mpb.Bar, wg *sync.WaitGroup, errChan chan error, table *model.Table, limit, page int32) {
    defer wg.Done()
